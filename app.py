...
# Importation of our backend logic
...
# Streamlit code
st.set_page_config(page_title="Grid07 AI Cognitive Simulator", layout="wide")
...
tab1, tab2, tab3 = st.tabs(
    ["Phase 1: Vector Routing", "Phase 2: Autonomous Content", "Phase 3: Combat Engine"]
)
...
with tab1:
    ...
    st.markdown(
        "When a parent post arrives, we embed it mathematically and query FAISS to find the bots that 'care' about this topic."
    )
    ...
    with col2:
        if route_btn:
            with st.spinner("Embedding & querying FAISS..."):
                matches = router.route_post_to_bots(
                    post_input, threshold=threshold_slider
                )

            if not matches:
                st.warning(
                    "No bots triggered. Try lowering the threshold or changing the topic."
                )
            else:
                st.success(f"Matched {len(matches)} bots!")
                for m in matches:
                    with st.expander(
                        f"{m['bot_id']} (Sim: {m['similarity']})", expanded=True
                    ):
...
with tab3:
    st.header("Phase 3: The Combat Engine (Deep Thread RAG)")
    st.markdown(
        """
    ...
    if prompt:
        if not os.getenv("GROQ_API_KEY"):
            ...
            st.session_state.messages.append(
                {"role": "user", "author": "Human", "content": prompt}
            )
            st.rerun()
    ...
    if (
        ...
        with st.chat_message("assistant"):
            ...
            st.write(response)
            st.session_state.messages.append(
                {"role": "assistant", "author": "Bot A", "content": response}
            )
